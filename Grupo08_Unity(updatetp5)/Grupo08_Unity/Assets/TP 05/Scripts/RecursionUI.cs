...
public class RecursionUI : MonoBehaviour
{
    ...
    {
        // 1️ Intenta convertir el texto del InputField a un número entero
        //    Si falla, 'int.TryParse' devuelve false.
        if (int.TryParse(inputField.text, out int cantidad))
        {
            // 2️ Si el usuario pone un número negativo, mostramos error.
            if (cantidad < 0)
            {
            ...
            // 3️ Creamos un string vacío para concatenar la secuencia.
            string resultado = "";

            // 4️ Usamos un bucle para calcular cada número de la secuencia
            ...
            // 5️ Mostramos en el Text el resultado
...
private string ConstruirPiramideRec(int nivel, int altura)
{
    ...
    private bool LimpiarTexto(string texto)
    {
        // Convertimos todo a minúsculas
        texto = texto.ToLower();

        // Construimos un string sin espacios
        string limpio = "";
        ...
        return EsPalindromo(limpio, 0, limpio.Length - 1);
    }


    private bool EsPalindromo(string s, int izquierda, int derecha)
    {
        if (izquierda >= derecha) return true; // caso base: llegó al centro
        if (s[izquierda] != s[derecha]) return false; // caso base: no coinciden
        return EsPalindromo(s, izquierda + 1, derecha - 1); // caso recursivo
    }
